...
class Sequencer:
    ...
    def run(self):
        startTime = time.time()
        result, usedOligos, sqpe = self._build_with_backtrack(self.start, self.initial_S1.copy(),
                                                              self.initial_S2.copy(), 0, 0)
        ...
        while len(sequence) < self.N:
            ...
            if len(matches) == 1:
                ...
                for c1, c2 in matches:
                    ...
                    if result:
                        return result, usedOligos, sqpe
                return None

        sqpe += len(S1)
        sqpe += len(S2)
        return sequence, usedOligos, sqpe
